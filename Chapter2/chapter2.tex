...
Nhằm tối ưu hóa hiệu năng hệ thống và bảo đảm tính bền vững, một cơ chế dự phòng (Fallback) tự động được thiết lập: ban đầu hệ thống sẽ ưu tiên sử dụng luồng cào tĩnh thông qua Requests và cloudscraper để đạt tốc độ tối đa \cite{ref13}; trong trường hợp hệ thống bảo mật của trang đích nâng cấp khiến luồng cào tĩnh bị chặn hoàn toàn, hệ thống sẽ tự động kích hoạt luồng dự phòng, chuyển đổi sang Selenium Headless Browser được cấu hình với các plugin chống phát hiện (như Undetected ChromeDriver), đồng thời áp dụng cơ chế giãn cách thời gian truy cập ngẫu nhiên và xoay vòng địa chỉ IP qua proxy để tiếp tục duy trì hoạt động thu thập dữ liệu tuyển dụng \cite{ref6}.
